...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { const j = await res.json(); throw new Error(j.error || 'Credenciais inválidas'); }
      router.push('/admin');
      router.refresh();
    } catch (err: unknown) {
        ...
        <div className="card p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input className="input-field" value={username} onChange={(e) => setUsername(e.target.value)} autoComplete="username" required />
            </div>
            <div>
            ...
            {error && <p className="text-red-600 text-sm bg-red-50 p-3 rounded-lg">⚠️ {error}</p>}
