...
function EuroField({ cents, onChange, placeholder = '0,00' }: {
  cents: number;
  ...
  useEffect(() => {
    fetch('/api/admin/settings')
      ...
      .then((data: SystemSettings) => {
        setSettings({ freightEurCents: data.freightEurCents ?? 0, minOrderEurCents: data.minOrderEurCents ?? 8500, flavorConfigs: data.flavorConfigs ?? [] });
        setLoading(false);
      })
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <EuroField cents={settings.minOrderEurCents} onChange={(c) => setSettings((p) => ({ ...p, minOrderEurCents: c }))} placeholder="85,00" />
          ...
          {settings.flavorConfigs.length === 0 ? (
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-[28px_1fr_110px_90px] gap-3 px-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wide">
                <div></div><div>Nome</div><div className="text-right">Preço</div><div className="text-center">Ações</div>
              </div>
                  ...
                  {editingId === fc.id ? (
                    <div className="grid grid-cols-[28px_1fr_110px_90px] gap-3 items-center p-3 rounded-xl border-2 border-brand-400 bg-brand-50">
                      ...
                      <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)}
                        className="input-field py-1.5 text-sm font-bold" autoFocus
                    ...
                    <div className={`grid grid-cols-[28px_1fr_110px_90px] gap-3 items-center p-3 rounded-xl border transition-all ${fc.active ? 'border-gray-200 bg-white' : 'border-dashed border-gray-200 bg-gray-50'}`}>
                      <input type="checkbox" checked={fc.active} onChange={() => update(fc.id, { active: !fc.active })}
                        className="accent-brand-600 w-4 h-4 cursor-pointer" title={fc.active ? 'Ocultar do site' : 'Mostrar no site'} />
                      ...
                      <div className="flex gap-1 justify-center">
                        <button onClick={() => startEdit(fc)} className="bg-blue-50 hover:bg-blue-100 text-blue-600 text-xs font-semibold px-2 py-1.5 rounded-lg" title="Editar">✏️</button>
                        <button onClick={() => remove(fc.id)} className="bg-red-50 hover:bg-red-100 text-red-500 text-xs font-semibold px-2 py-1.5 rounded-lg" title="Excluir">🗑️</button>
                      </div>
                    </div>
                  )}
                </div>
