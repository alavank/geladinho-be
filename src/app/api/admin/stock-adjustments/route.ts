...
export async function GET(request: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: 'Erro ao buscar ajustes' }, { status: 500 });
...
export async function POST(request: NextRequest) {
  ...
  if (!body.quantity || body.quantity === 0) {
    return NextResponse.json({ error: 'Quantidade não pode ser zero' }, { status: 400 });
  }
  if (!body.reason) {
  ...
  if (error) return NextResponse.json({ error: 'Erro ao registrar ajuste' }, { status: 500 });
