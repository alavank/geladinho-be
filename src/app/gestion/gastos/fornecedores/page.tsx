...
export default function FornecedoresPage() {
  ...
  const fetchSuppliers = async () => {
    const res = await fetch('/api/gestion/suppliers');
  ...
  const handleSave = async () => {
    ...
    const url = editingId ? `/api/gestion/suppliers/${editingId}` : '/api/gestion/suppliers';
  ...
  const handleToggle = async (s: Supplier) => {
    await fetch(`/api/gestion/suppliers/${s.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    const res = await fetch(`/api/gestion/suppliers/${id}`, { method: 'DELETE' });
