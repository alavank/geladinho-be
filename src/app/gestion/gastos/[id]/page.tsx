...
export default function VisualizarGastoPage() {
  ...
  useEffect(() => {
    fetch(`/api/gestion/expenses/${id}`)
      .then((r) => {
        if (r.status === 401) { router.push('/gestion/login'); throw new Error('unauth'); }
        if (!r.ok) throw new Error('not-found');
        return r.json();
      })
      .then(setExpense)
      .catch((e: Error) => {
        if (e.message === 'not-found') router.push('/gestion/gastos', { scroll: false });
      })
  ...
  const handleDelete = async () => {
    ...
    if (res.ok) router.push('/gestion/gastos', { scroll: false });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <AdminHeader
        ...
        actions={
          <>
            <Link href={`/gestion/gastos/${id}/editar`} scroll={false} className="btn-primary py-1.5 px-4 text-sm">Editar</Link>
      ...
      <div className="mx-auto max-w-3xl px-4 py-6 space-y-5">
        ...
        <div className="pt-2 pb-8">
          <Link href="/gestion/gastos" scroll={false}
