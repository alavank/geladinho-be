...
export default function EstoquePage() {
  ...
  const isActiveModule = useIsAdminModuleActive('/gestion/estoque');
  ...
  const fetchAll = useCallback(async (force = false) => {
    ...
    const [sRes, pRes, aRes] = await Promise.all([
      fetch('/api/gestion/stock'),
      fetch('/api/gestion/production'),
      fetch('/api/gestion/stock-adjustments'),
    ]);
    if (sRes.status === 401) { router.push('/gestion/login'); return; }
  ...
  const handleAddProduction = async () => {
    ...
    setProdSaving(true);
    const res = await fetch('/api/gestion/production', {
      method: 'POST',
  ...
  const handleAddAdjustment = async () => {
    ...
    const res = await fetch('/api/gestion/stock-adjustments', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        date: adjDate,
  ...
  const handleDeleteBatch = async (id: string) => {
    ...
    const res = await fetch(`/api/gestion/production/${id}`, { method: 'DELETE' });
