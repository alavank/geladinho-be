...
export default function ConfiguracoesPage() {
  ...
  const isActiveModule = useIsAdminModuleActive('/gestion/configuracoes');
  ...
  const fetchSettings = useCallback(async (force = false) => {
    ...
    return fetch('/api/gestion/settings')
      .then((r) => { if (r.status === 401) { router.push('/gestion/login'); throw new Error('unauth'); } return r.json(); })
  ...
  const handleSave = async () => {
    ...
    const res = await fetch('/api/gestion/settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(settings) });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Link to B2B */}
        <Link href="/gestion/configuracoes/b2b" scroll={false}
