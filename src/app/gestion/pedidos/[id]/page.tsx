...
export default function OrderDetailPage() {
  ...
  const updateStatus = async (newStatus: OrderStatus) => {
    ...
    const response = await fetch(`/api/gestion/orders/${id}`, {
      method: 'PATCH',
  ...
  const handleDelete = async () => {
    ...
    const response = await fetch(`/api/gestion/orders/${id}`, { method: 'DELETE' });
    if (response.ok) {
      router.push('/gestion', { scroll: false });
    } else {
      const data = await response.json().catch(() => ({}));
  ...
  const saveFreight = async () => {
    ...
    const response = await fetch(`/api/gestion/orders/${id}/freight`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/gestion" scroll={false} className="btn-secondary">← Voltar</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
        <div className="flex flex-wrap gap-3">
          ...
          {!isB2B && (
            <button
              onClick={() => window.open(`/gestion/pedidos/${id}/bon`, '_blank')}
          ...
          {isB2B && (
            <button
              onClick={() => window.open(`/gestion/pedidos/${id}/bon-revenda`, '_blank')}
