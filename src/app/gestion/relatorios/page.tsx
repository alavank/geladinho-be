...
export default function RelatoriosPage() {
  ...
  const isActiveModule = useIsAdminModuleActive('/gestion/relatorios');
        ...
        .then(async (orderList: Order[]) => {
          return Promise.all(
            orderList.map(async (o) => {
              try { const r = await fetch(`/api/gestion/orders/${o.id}`); return await r.json(); } catch { return o; }
            })
          );
        }),
      fetch('/api/gestion/expenses').then((r) => r.ok ? r.json() : []),
