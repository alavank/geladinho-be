...
export default function OrderForm({ initialData, onSubmit, cartItems, totalUnits, subtotalCents, freightCents, grandTotalCents }: Props) {
        ...
        <div className="card p-5 mb-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input className={`input-field ${err('customerName')}`} placeholder="Ex: Maria Silva"
            ...
            <div>
              ...
              <input className={`input-field ${err('customerPhone')}`} placeholder="+32 470 12 34 56"
                value={form.customerPhone} onChange={(e) => set('customerPhone', e.target.value)} type="tel" />
        ...
        <div className="card p-5 mb-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input className={`input-field ${err('addressStreet')}`} placeholder="Ex: Rue de la Loi"
            ...
            <div>
              ...
              <input className={`input-field ${err('addressNumber')}`} placeholder="Ex: 16"
            ...
            <div>
              ...
              <input className={`input-field ${err('addressPostalCode')}`} placeholder="Ex: 1000"
                value={form.addressPostalCode} onChange={(e) => set('addressPostalCode', e.target.value.replace(/\D/g, ''))}
                maxLength={4} inputMode="numeric" />
              {errors.addressPostalCode && <p className="text-red-500 text-xs mt-1">{errors.addressPostalCode}</p>}
            </div>
            <div>
              <label className="label">Comuna *</label>
              <input className={`input-field ${err('addressCommune')}`} placeholder="Ex: Bruxelles, Liège, Anderlecht..."
                value={form.addressCommune} onChange={(e) => set('addressCommune', e.target.value)} />
        ...
        <div className="card p-5 mb-5">
          ...
          <div className="flex gap-3 mb-4">
            {[{ value: false, label: 'Não', icon: '✅' }, { value: true, label: 'Sim', icon: '💵' }].map((opt) => (
              <label key={String(opt.value)} className={`flex-1 flex items-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-all ${
                form.needsChange === opt.value ? 'border-brand-500 bg-brand-50' : 'border-gray-200 hover:border-gray-300'
              }`}>
                <input type="radio" name="needsChange" checked={form.needsChange === opt.value}
                  onChange={() => set('needsChange', opt.value)} className="accent-brand-600" />
          ...
          {form.needsChange && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              ...
              <input className={`input-field mt-1 ${err('changeAmount')}`} placeholder="Ex: 50 ou 100"
                value={form.changeAmount} onChange={(e) => set('changeAmount', e.target.value)} inputMode="decimal" />
        ...
        <div className="card p-4 mb-6 bg-brand-50 border border-brand-200">
          <div className="space-y-1">
            ...
            <div className="flex justify-between font-bold text-lg text-brand-700 border-t border-brand-200 pt-1">
              <span>Total</span>
              <span>{formatEUR(grandTotalCents)}</span>
            </div>
          </div>
        </div>
