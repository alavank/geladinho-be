...
import { Flavor, getFlavorColor, formatEUR } from '@/lib/flavors';
...
export default function FlavorCard({ flavor, index, quantity, unitPriceCents, onQuantityChange }: Props) {
  ...
  return (
    <div className={`rounded-2xl border-2 transition-all duration-200 flex flex-col overflow-hidden ${
      ...
      <div className={`${colorClass} flex-1 flex flex-col items-center justify-center px-3 py-6 min-h-[100px]`}>
        <p className="font-black text-center leading-tight text-base tracking-wide">{flavor.name}</p>
