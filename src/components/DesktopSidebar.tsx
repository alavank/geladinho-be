...
export default function DesktopSidebar({ cartItems, totalUnits, subtotalCents, freightCents, grandTotalCents, minOrderCents, onProceed }: Props) {
  ...
  return (
    <div className="hidden lg:block w-80 shrink-0">
      <div className="sticky top-24 card p-5">
        <h3 className="font-bold text-lg text-gray-900 mb-4 flex items-center gap-2">🛒 Resumo do Pedido</h3>
        ...
        {!canProceed && totalUnits > 0 && (
          <p className="text-xs text-amber-600 bg-amber-50 rounded-lg p-2 mt-3 text-center">
            Faltam <strong>{formatEUR(remaining)}</strong> para o pedido mínimo de {formatEUR(minOrderCents)}
          </p>
        )}
